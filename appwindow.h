...
inline auto MainWindow::item_geometry(uint32_t index) const -> slint::cbindgen_private::Rect
{
    ...
    switch (index) {
    case 0:
        return slint::private_api::convert_anonymous_rect(std::make_tuple(float(self->root_1.height.get()), float(self->root_1.width.get()), float(0), float(0)));
    case 1:
        return slint::private_api::convert_anonymous_rect(std::make_tuple(float(self->text_2.height.get()), float(self->text_2.width.get()), float(self->root_1_text_2_x.get()), float(self->root_1_text_2_y.get())));
    }
    return {};
}
...
inline auto MainWindow::accessible_role(uint32_t index) const -> slint::cbindgen_private::AccessibleRole
{
    ...
    switch (index) {
    case 1:
        return slint::cbindgen_private::AccessibleRole::Text;
    }
    return {};
}
...
inline auto MainWindow::accessible_string_property(uint32_t index, slint::cbindgen_private::AccessibleStringProperty what) const -> std::optional<slint::SharedString>
{
    ...
    case (1 << 8) | uintptr_t(slint::cbindgen_private::AccessibleStringProperty::Label):
...
{
    if (auto r = reinterpret_cast<const MainWindow*>(component.instance)->accessible_string_property(index, what)) {
        *result = *r;
        return true;
    } else {
        return false;
...
{
    if (auto& window = globals->m_window)
        window->window_handle().unregister_item_tree(this, item_array());
}
