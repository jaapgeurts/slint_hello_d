...
class AbstractRenderer {
    ...
    /// \private
    abstract RendererPtr renderer_handle() const;
...
class WindowAdapter {
    ...
    WindowAdapterRcOpaque self;
    // Whether this WindowAdapter was already given to the slint runtime
    bool was_initialized = false;

    WindowAdapterRcOpaque initialize() {
        slint_window_adapter_new(this, (void* wa) {
            delete cast(WindowAdapter*)(wa);
        }, (void* wa) {
            return cast(WindowAdapter*)(wa).renderer().renderer_handle();
        }, (void* wa, bool visible) {
            cast(WindowAdapter*)(wa).set_visible(visible);
        }, (void* wa) { cast(WindowAdapter*)(wa).request_redraw(); }, (void* wa) {
            return cast(IntSize) cast(WindowAdapter*)(wa).size();
        }, (void* wa, IntSize size) {
            cast(WindowAdapter*)(wa).set_size(PhysicalSize(size.width, size.height));
        }, (void* wa, const WindowProperties* p) {
            cast(WindowAdapter*)(wa).update_window_properties(*cast(const WindowProperties*)(p));
        }, (void* wa, Point2D!(int32_t)* point) {
            if (auto pos = cast(WindowAdapter*)(wa).position()) {
                *point = *pos;
                return true;
            }
            else {
            ...
            cast(WindowAdapter*)(wa).set_position(PhysicalPosition(point.x, point.y));
        }, &self);
...
public:
    ...
    /// to the slint view.
    void set_size(PhysicalSize) {
    }

    /// Returns the actual physical size of the window
    abstract PhysicalSize size();
    ...
    void set_position(PhysicalPosition) {
    }
    ...
    Nullable!PhysicalPosition position() {
        return nullable!PhysicalPosition;
    }
    ...
    struct WindowProperties {
        ...
        SharedString title() const {
            SharedString out_;
            slint_window_properties_get_title(inner(), &out_);
            return out_;
        }
        ...
        // {
        //     Brush out;
        //     slint_window_properties_get_background(inner(), &out);
        //     return out;
        // }

        /// \deprecated Use is_fullscreen() instead
        // [[deprecated("Renamed is_fullscreen()")]]
        ...
        bool is_fullscreen() const {
            return slint_window_properties_get_fullscreen(inner());
        }

        /// Returns true if the window should be minimized; false otherwise
        bool is_minimized() const {
            return slint_window_properties_get_minimized(inner());
        }

        /// Returns true if the window should be maximized; false otherwise
        bool is_maximized() const {
            return slint_window_properties_get_maximized(inner());
        }
        ...
        LayoutConstraints layout_constraints() const {
            auto lc = slint_window_properties_get_layout_constraints(inner());
            return LayoutConstraints(lc.has_min ? nullable(LogicalSize(lc.min)) : nullable!LogicalSize,
                    lc.has_max ? nullable(LogicalSize(lc.max)) : nullable!LogicalSize,
                    LogicalSize(lc.preferred));
        }

    private:
        const(WindowProperties)* inner() const {
            return cast(const WindowProperties*)(this);
        }
    }
    ...
    void update_window_properties(const WindowProperties) {
    }
    ...
    abstract ref AbstractRenderer renderer();
    ...
    Window window() const {
        if (!was_initialized)
            abort();
        // This works because WindowAdapterRcOpaque and Window have the same
        // layout
        return cast(const Window*)(&self);
    }
...
//     std::unique_ptr<WindowAdapter> create_window_adapter() = 0;
//
...
//     std::chrono::milliseconds duration_since_start() = 0;
// #endif
//
...
//         DefaultClipboard = static_cast<uint8_t>(Clipboard::DefaultClipboard),
...
//     class Task
//     {
//         PlatformTaskOpaque inner { nullptr, nullptr };
//         friend inline void set_platform(std::unique_ptr<Platform> platform);
//         explicit Task(PlatformTaskOpaque inner) : inner(inner) { }
//
//     public:
//         ~Task()
//         {
//             if (inner._0) {
//                 slint_platform_task_drop(
...
// #endif
//             },
//             [](void *p, const SharedString *text, Clipboard clipboard) {
//                 reinterpret_cast<Platform *>(p)->set_clipboard_text(
//                         *text, static_cast<Platform::Clipboard>(clipboard));
//             },
//             [](void *p, SharedString *out_text, Clipboard clipboard) -> bool {
...
//             [](void *p, PlatformTaskOpaque event) {
...
// {
//     mutable SoftwareRendererOpaque inner;
//
//     /// \private
//     RendererPtr renderer_handle() const override
//     {
//         return slint_software_renderer_handle(inner);
//     }
//
// public:
...
//         {
//             SharedVector<IntRect> rectangles;
...
//             {
//                 SharedVector<IntRect> rectangles;
//                 owning_view(SharedVector<IntRect> &&rectangles)
...
//
//     private:
//         PhysicalRegion inner;
//         friend class SoftwareRenderer;
//         PhysicalRegion(PhysicalRegion inner) : inner(std::move(inner)) { }
//     };
//
...
//     using DrawTextureArgs = DrawTextureArgs;
//     /// Arguments for draw_rectagle
//     using DrawRectangleArgs = DrawRectangleArgs;
//
...
//         std::span<PixelType> line_slice(std::size_t line_number) = 0;
...
//         bool draw_texture(const DrawTextureArgs &texture, const PhysicalRegion &clip) = 0;
//
//         /// Fill the background of the buffer with the given brush.
//         bool fill_background(const Brush &brush, const PhysicalRegion &clip) = 0;
//
//         /// Draw a rectangle specified by the DrawRectangleArgs. That rectangle must be clipped to
//         /// the given region.
//         bool draw_rectangle(const DrawRectangleArgs &args, const PhysicalRegion &clip) = 0;
//
//     private:
//         friend class SoftwareRenderer;
//         CppTargetPixelBuffer<PixelType> wrap()
//         {
//             return CppTargetPixelBuffer<PixelType> {
//                 .user_data = this,
//                 .line_slice =
...
//                         },
//                 .draw_texture =
//                         [](void *self, const DrawTextureArgs *texture,
//                            const PhysicalRegion *clip) {
//                             auto *buffer = reinterpret_cast<TargetPixelBuffer<PixelType> *>(self);
//                             auto clip_region = PhysicalRegion { *clip };
...
// #    endif
//
//     ~SoftwareRenderer() { slint_software_renderer_drop(inner); };
...
//     {
//         inner = slint_software_renderer_new(uint32_t(buffer_type));
//     }
//
...
//     {
//         auto r = slint_software_renderer_render_rgb8(inner, buffer.data(),
...
//     {
//         auto r = slint_software_renderer_render_rgb565(
...
//             return PhysicalRegion { slint_software_renderer_render_by_line_rgb565(
//                     inner, process_line_fn, &process_line_callback) };
//         } else if constexpr (std::is_same_v<PixelType, Rgb8Pixel>) {
//             return PhysicalRegion { slint_software_renderer_render_by_line_rgb8(
//                     inner, process_line_fn, &process_line_callback) };
//         } else {
...
//         auto r = slint_software_renderer_render_accel_rgb8(inner, &wrapper);
//         return PhysicalRegion { r };
//     }
//
//     /// Renders into the given TargetPixelBuffer.
...
//     {
//         auto wrapper = buffer->wrap();
//         auto r = slint_software_renderer_render_accel_rgb565(inner, &wrapper);
//         return PhysicalRegion { r };
//     }
// #    endif
//
...
//     {
//         slint_software_renderer_set_rendering_rotation(
...
// {
//     CppRawHandleOpaque inner;
//     friend class SkiaRenderer;
//
//     NativeWindowHandle(CppRawHandleOpaque inner) : inner(inner) { }
//
// public:
...
//         }
//         if (inner) {
//             slint_raw_window_handle_drop(inner);
//         }
...
//     {
//
//         return { slint_new_raw_window_handle_x11_xcb(window, visual_id,
//                                                                        connection, screen) };
//     }
//
...
//     {
//
//         return { slint_new_raw_window_handle_x11_xlib(window, visual_id, display,
//                                                                         screen) };
//     }
//
...
//     {
//
//         return { slint_new_raw_window_handle_wayland(surface, display) };
//     }
//
// #    endif
// #    if (defined(__APPLE__) && !defined(_WIN32) && !defined(_WIN64)) || defined(DOXYGEN)
...
//     {
//
//         return { slint_new_raw_window_handle_appkit(nsview, nswindow) };
//     }
//
// #    endif
// #    if (!defined(__APPLE__) && (defined(_WIN32) || !defined(_WIN64))) || defined(DOXYGEN)
...
//     {
//         return { slint_new_raw_window_handle_win32(hwnd, hinstance) };
//     }
// #    endif
...
// {
//     mutable SkiaRendererOpaque inner;
//
//     /// \private
//     RendererPtr renderer_handle() const override
//     {
//         return slint_skia_renderer_handle(inner);
//     }
//
// public:
//     ~SkiaRenderer() { slint_skia_renderer_drop(inner); }
...
//     {
//         inner = slint_skia_renderer_new(window_handle.inner, initial_size);
//     }
//
//     /// Renders the scene into the window provided to the SkiaRenderer's constructor.
//     void render() const { slint_skia_renderer_render(inner); }
// };
// #endif
//
...
// {
//     uint64_t val = slint_platform_duration_until_next_timer_update();
